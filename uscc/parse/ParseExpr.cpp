...
shared_ptr<ASTLogicalOr> Parser::parseExprPrime(shared_ptr<ASTExpr> lhs)
{
	...
	if (peekToken() == Token::Or)
	{
		...
		// PA2: Finalize op
		if (lhs->getType() != Type::Void && rhs->getType() != Type::Void)
		{
			if (!retVal->finalizeOp())
			{
				std::string lhsType(getTypeText(lhs->getType()));
				std::string rhsType(getTypeText(rhs->getType()));
				reportSemantError("Cannot perform op between type " + lhsType + " and " + rhsType, col);
			}
		}
...
shared_ptr<ASTStringExpr> Parser::parseStringFactor()
{
	...
	if (peekToken() == Token::String)
	{
		...
		retVal = make_shared<ASTStringExpr>(tempStr, mStrings);
		consumeToken();
	}
	return retVal;
}
...
shared_ptr<ASTExpr> Parser::parseAddrOfArrayFactor()
{
	...
	if (peekAndConsume(Token::Addr))
	{
		try
		{
			if (peekToken() == Token::Identifier || mUnusedArray != nullptr)
			{
				if (mUnusedArray)
				{
					retVal = make_shared<ASTAddrOfArray>(mUnusedArray);
					mUnusedArray = nullptr;
				}
				else
				{
